...
@store = Redis.new
  ...
  # puts 'Let's get gold price!'
   ...
   usd_rate = probe_arr(@gold_value, mark_gold).to_h
     ...
     @store.hset("KGS", :"#{key}", value * @currency_kgs.to_i)
     @store.hset("KZT", :"#{key}", value * @currency_kzt.to_i)
     @store.hset("USD", key, value)
     end

puts 'Done!'
puts @store.hgetall("KZT")
#puts @store.hgetall("KGS")
#puts @store.hgetall("USD")
end
